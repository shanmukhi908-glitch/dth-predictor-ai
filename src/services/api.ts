...
export async function predictDaysToHeading(data: CropInput): Promise<PredictionResult> {
  ...
  try {
    ...
    // In production, strictly reject fake predictions and surface the actual error
    if (isProduction) {
      ...
      throw new Error(err.message || 'Unable to connect to the prediction API server. Please check your backend URL and status.');
    }

    // In local development ONLY, allow graceful mock fallback if developer hasn't started the backend yet
    console.warn('Backend endpoint unreachable in development mode; executing local fallback.', err);
...
function parseApiResponse(backendRes: any, input: CropInput): PredictionResult {
  ...
  return {
    ...
    confidencePercentage: 90.76,
    maturityCategory:
      roundedDTH < 170 ? 'Early Season (< 170 Days)' : roundedDTH <= 178 ? 'Optimal Mid-Season (170 - 178 Days)' : 'Late Season (> 178 Days)',
    phenologicalStage: 'Heading / Inflorescence Emergence',
    explanation:
